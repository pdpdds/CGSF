...
#define MAX_VIEW_SIZE	10485760
	...
	bool GetFileSizeQW(const HANDLE hFile, __int64 &qwSize)
	{
...
DWORD SFChecksum::FromBuffer(BYTE* buffer, size_t size, DWORD &dwCrc32)
{
	_ASSERTE(buffer);
	...
	try
	{
		...
		{
			Private::CalcCrc32((BYTE)*buffer, dwCrc32);
			++buffer;
		}
	}
	catch (...)
...
DWORD SFChecksum::FromFileWin32(LPCWSTR filename, DWORD &dwCrc32)
{
	_ASSERTE(filename);
		...
		else
		{
			...
			while(bSuccess && dwBytesRead)
			{
				for(dwLoop = 0; dwLoop < dwBytesRead; dwLoop++)
					Private::CalcCrc32(buffer[dwLoop], dwCrc32);
...
DWORD SFChecksum::FromFileFilemap(LPCWSTR filename, DWORD &dwCrc32)
{
	_ASSERTE(filename);
	_ASSERTE(lstrlenW(filename));
	...
	try
	{
		...
		else
		{
			...
			// Get the file size
			if(!Private::GetFileSizeQW(hFile, qwFileSize))
			...
			else if(qwFileSize != 0)	// We cannot CRC zero byte files
			{
				...
				else
				{
					...
					while (qwFileSize > 0)
					{
						...
						while (dwByteCount-- > 0)
						{
							Private::CalcCrc32(*pByte, dwCrc32);
							pByte++;
						}
