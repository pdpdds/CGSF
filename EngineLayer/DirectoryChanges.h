...
public:
	...
	const CStdString & GetChangedDirectoryName() const { return m_strChangedDirectoryName;}//WARNING: don't use this, this function will be removed in a future release.
...
protected:
	...
	virtual void On_FileAdded(const CStdString & strFileName); //=0;
			//
			//	On_FileAdded()
			//
			...
			//
	virtual void On_FileRemoved(const CStdString & strFileName);// = 0;
			//
			//	On_FileRemoved()
			//
			//	This function is called when a file in one of the watched folders(or subfolders)
	...
	virtual void On_FileNameChanged(const CStdString & strOldFileName, const CStdString & strNewFileName);// = 0;
			//
			//	On_FileNameChanged()
			//
			//	This function is called when a file in one of the watched folders(or subfolders)
	...
	virtual void On_FileModified(const CStdString & strFileName);// = 0;
			//
			//	On_FileModified()
			//
	...
	virtual void On_ReadDirectoryChangesError(DWORD dwError, const CStdString & strDirectoryName);
			//
	...
	virtual void On_WatchStarted(DWORD dwError, const CStdString & strDirectoryName);
			//
			...
			//	const CStdString & strDirectoryName -- The full path and name of the directory being watched.
	
	virtual void On_WatchStopped(const CStdString & strDirectoryName);
			//
	...
	void SetChangedDirectoryName(const CStdString & strChangedDirName);//please don't use this function, it will be removed in future releases.
	
private:
	long m_nRefCnt;
	
	CStdString m_strChangedDirectoryName;//will be removed in a future release.
...
public:
	...
	//
	DWORD	WatchDirectory(const CStdString & strDirToWatch, 
	...
	BOOL	IsWatchingDirectory (const CStdString & strDirName)const;
	int		NumWatchedDirectories()const; //counts # of directories being watched.

	
	BOOL	UnwatchDirectory(const CStdString & strDirToStopWatching);//stops watching specified directory.
...
protected:
	...
	public:
		CDirWatchInfo(HANDLE hDir, const CStdString & strDirectoryName, 
			CDirectoryChangeHandler * pChangeHandler, 
	...
	public:
		BOOL LockProperties() { m_cs.Lock(); return TRUE; }
		BOOL UnlockProperties(){ m_cs.Unlock(); return TRUE;	}
		...
		BOOL		m_bWatchSubDir;
		CStdString     m_strDirName;//name of the directory that we're watching
...
private:
	...
	UINT static __stdcall MonitorDirectoryChanges(LPVOID lpvThis );//the worker thread that monitors directories.
	...
	//
	CDirWatchInfo *	GetDirWatchInfo(IN const CStdString & strDirName, OUT int & ref_nIdx)const;
