...
public :
	...
	TiXmlString (const WCHAR * instring);
	...
	// Convert a TiXmlString into a classical WCHAR *
	...
	void operator = (const WCHAR * content);

	// = operator
	void operator = (const TiXmlString & copy);

	// += operator. Maps to append
	TiXmlString& operator += (const WCHAR * suffix)
	{
		append (suffix);
		return *this;
	}
	...
	// += operator. Maps to append
	TiXmlString& operator += (WCHAR single)
	{
		append (single);
		return *this;
	}
	...
	// single WCHAR extraction
	const WCHAR& at (unsigned index) const
	{
		assert( index < length ());
		return cstring [index];
	}

	// find a WCHAR in a string. Return TiXmlString::notfound if not found
	unsigned find (WCHAR lookup) const
	{
		return find (lookup, 0);
	}

	// find a WCHAR in a string from an offset. Return TiXmlString::notfound if not found
	unsigned find (WCHAR tofind, unsigned offset) const;
	...
	*/
	...
	// [] operator 
	WCHAR& operator [] (unsigned index) const
	{
		assert( index < length ());
		return cstring [index];
	}
	...
	void append (const WCHAR *str, int len );
...
protected :
	...
	void append (const WCHAR *suffix );
	...
	// append for a single WCHAR. This could be improved a lot if needed
	void append (WCHAR single)
	{
		WCHAR smallstr [2];
		smallstr [0] = single;
		smallstr [1] = 0;
...
public :
	...
	TiXmlOutStream & operator << (const WCHAR * in)
	{
		append (in);
		return (* this);
	}
