...
void ChatInputThread(void* Args)
{
	std::string input;
	...
	while (SFNetworkEntry::GetInstance()->IsConnected())
	{
		...
		SFJsonPacket packet(CHAT_PACKET_NUM);
...
void ProcessInput()
{
	int inputThreadID = ACE_Thread_Manager::instance()->spawn_n(1, (ACE_THR_FUNC)ChatInputThread, NULL, THR_NEW_LWP, ACE_DEFAULT_THREAD_PRIORITY);
