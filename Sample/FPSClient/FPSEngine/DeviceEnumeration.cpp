...
INT_PTR DeviceEnumeration::SettingsDialogProc( HWND dialog, UINT msg, WPARAM wparam, LPARAM lparam )
{
	switch( msg )
	{
		case WM_INITDIALOG:
		{
			...
			SetWindowTextA( GetDlgItem( dialog, IDC_DISPLAY_ADAPTER ), m_adapter.Description );
			SetWindowTextA( GetDlgItem( dialog, IDC_DRIVER_VERSION ), version );
