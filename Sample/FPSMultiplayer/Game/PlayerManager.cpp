...
//		rspm.msgid = MSGID_SPAWN_POINT_REQUEST;
	...
	if( lookUpdate + 100 < timeGetTime() && localPlayer->GetEnabled() == true )
	{
		...
		g_engine->GetNetwork()->UDPSend((unsigned char*)(&plum), sizeof(PlayerLookUpdateMsg));
		//g_engine->TCPSend(CGSF::MSG_PLAYER_LOOK_UPDATE, &plum, sizeof(PlayerLookUpdateMsg));
	...
	if( ( m_localMovement == true || moveUpdate + 200 < timeGetTime() ) && localPlayer->GetEnabled() == true )
	{
		...
		g_engine->GetNetwork()->UDPSend((unsigned char*)(&pmum), sizeof(PlayerMoveUpdateMsg));
